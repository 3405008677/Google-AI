...
動態模型選擇：
Supervisor 會根據 user_context 自動選擇對應的 AI 模型：
...
class SupervisorConfig:
    ...
    注意：模型選擇已改為動態方式，從 user_context 中讀取。
    這裡只保留 Supervisor 行為相關的配置。
    """
    temperature: float = 0.0  # Supervisor 使用低溫度以確保決策穩定
...
# 提示詞現在從配置文件讀取：src/common/prompts/config.yaml
# 使用 get_prompt("supervisor.planning") 和 get_prompt("supervisor.routing") 獲取


def _build_planning_prompt(worker_list: str, max_steps: int) -> ChatPromptTemplate:
    """构建任务规划 Prompt（從配置文件讀取）"""
    # 從配置文件獲取提示詞，支持模板變量
    ...
    planning_complete = get_prompt(
        ...
        default='請分析用戶的請求，制定一個執行計劃。返回 JSON 格式：{{"steps": [{{"worker": "專家名稱", "description": "任務描述"}}], "reasoning": "規劃理由"}}'
    )
...
def _build_routing_prompt(
    ...
    routing_decision = get_prompt(
        ...
        default="根據以上對話歷史和任務進度，請做出你的決策：下一步交給哪個專家？或者任務是否已經完成？"
    )
...
def _get_llm_from_state(state: SupervisorState, temperature: float = 0.0) -> BaseChatModel:
    ...
    根據 user_context 動態選擇對應的模型。
    Supervisor 使用較低溫度以確保決策穩定。
    """
    import os
...
def create_supervisor_node(
    ...
    動態模型選擇：
    LLM 會根據 state["user_context"] 動態選擇對應的模型。
    這允許不同路由（Customize、Qwen、Gemini）使用各自配置的模型。
    
    Args:
        config: Supervisor 配置，如果为 None 则使用默认配置
        llm: 可选的 LLM 实例，用于测试时注入 mock（覆蓋動態選擇）
    ...
    # 如果提供了 llm（用於測試），則使用它；否則會在每次請求時根據 state 動態創建
    _fixed_llm = llm
        ...
        try:
            ...
            if isinstance(result, TaskPlan):
                ...
                for i, step in enumerate(result.steps):
                    # 清理 Worker 名稱，移除可能的類型標記（如 "Researcher [llm_powered]" -> "Researcher"）
    ...
    async def _route_decision(state: SupervisorState, registry) -> Dict[str, Any]:
        ...
        for i, step in enumerate(task_plan):
            ...
            if not is_completed:
                ...
                # 處理 LLM 可能返回帶有類型標記的 Worker 名稱，如 "Researcher [llm_powered]"
                if "[" in next_worker:
                ...
                # 嘗試不區分大小寫匹配
                    ...
                    # Worker 名稱無效，使用 General 作為備選
                    logger.warning(f"計劃中的 Worker '{next_worker}' 不存在，使用 General 代替")
        ...
        try:
            ...
            if isinstance(result, RouteDecision):
                ...
                if next_action not in valid_options:
                    ...
                    # 嘗試從 reasoning 中智能提取正確的 Worker 名稱
                    ...
                    if fallback_worker:
                        logger.info(f"從 reasoning 中推斷出目標 Worker: {fallback_worker}")
                        next_action = fallback_worker
                    else:
                        # 如果還有未完成的任務步驟，使用計劃中的 Worker
                        for step in task_plan:
                            if step.get("status") not in [TaskStatus.COMPLETED, TaskStatus.SKIPPED, TaskStatus.FAILED]:
                                ...
                                if planned_worker in worker_names:
                                    logger.info(f"使用任務計劃中的 Worker: {planned_worker}")
                                    next_action = planned_worker
                                    break
                        else:
                            # 最終回退到 FINISH
                            logger.warning(f"無法推斷有效的 Worker，使用 FINISH")
                            next_action = "FINISH"
                
                # 關鍵檢查：如果 LLM 返回 FINISH 但還有未完成的任務，強制使用計劃中的 Worker
                if next_action == "FINISH" and completed_steps < total_steps:
                    logger.warning(f"LLM 返回 FINISH 但還有未完成任務 ({completed_steps}/{total_steps})，嘗試使用計劃中的 Worker")
                    for step in task_plan:
                        step_status = step.get("status")
                        if isinstance(step_status, str):
                            is_completed = step_status in ["completed", "skipped", "failed"]
                        ...
                        if not is_completed:
                            ...
                            if planned_worker in worker_names:
                                next_action = planned_worker
                                logger.info(f"強制使用計劃中的 Worker: {next_action}")
                                break
