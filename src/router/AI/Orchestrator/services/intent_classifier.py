...
意图类型说明：
1. chat（普通聊天）：日常对话、问答、闲聊、情感交流等不需要外部信息的对话。知识性问题如果可以用已有知识回答，也属于chat。
2. search（网络搜索）：需要获取最新信息、实时数据、当前时间/日期、新闻、技术文档、产品信息等需要从网络搜索的内容。
   **特别注意**：任何涉及"今天"、"现在"、"当前"、"最新"、"最近"等时间相关的问题，都需要search，因为需要获取实时信息。
...
示例：
用户："今天天气怎么样？"
返回：{"intent": "search", "confidence": 0.95, "reason": "需要获取实时天气信息"}

用户："今天几月几号？"
返回：{"intent": "search", "confidence": 0.95, "reason": "需要获取当前日期信息"}

用户："现在几点了？"
返回：{"intent": "search", "confidence": 0.95, "reason": "需要获取当前时间信息"}
...
返回：{"intent": "search", "confidence": 0.9, "reason": "需要获取最新版本信息"}"""
    ...
    def _fallback_classify(self, query: str) -> Dict:
        ...
        # 搜索相关关键词（包括时间相关）
        search_keywords = [
            "最新", "最近", "现在", "当前", "实时", "新闻", "搜索", "查找", "找", "查",
            "今天", "明天", "昨天", "几号", "几月", "星期", "周几", "几点", "什么时候",
            "天气", "温度", "日期", "时间", "现在", "此刻", "当前时间", "当前日期"
        ]
        if any(keyword in query_lower for keyword in search_keywords):
            return {
                "intent": IntentType.SEARCH.value,
                "confidence": 0.8,
                "reason": "检测到需要实时信息的关键词"
            }
